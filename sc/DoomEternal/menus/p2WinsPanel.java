...
public class p2WinsPanel extends JPanel {

    private final BufferedImage menuBackground;
    private final Launcher lf;


    public p2WinsPanel(Launcher lf) {
        this.lf = lf;

        int random = (int) (Math.random() * 2 + 1);

        if(random == 1){
            menuBackground = GatheredResources.getImage("baronWin1");
        }else {
            menuBackground = GatheredResources.getImage("baronWin2");
        }
